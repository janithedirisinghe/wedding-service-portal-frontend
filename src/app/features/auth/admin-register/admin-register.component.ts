...
export class AdminRegisterComponent implements OnInit {
  ...
  onOtpVerificationComplete() {
    this.router.navigate(['/admin']); // Navigate to admin dashboard after successful registration
  }

  registerAdmin(event?: Event) {
    ...
    const registrationData: Customer = {
      ...formValues,
      role: 'ADMIN' // Set role to ADMIN instead of CUSTOMER
    };
    ...
    this.customerService.registerCustomer(registrationData).subscribe(
      (response: any) => {
        if (response.message === 'User registered successfully!') {
          ...
          setTimeout(() => {
            this.toastr.success('Admin registration successful! OTP sent to your email', 'Success');
          }, 100);
          this.spinner.hide();
